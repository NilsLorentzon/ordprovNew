  ...
  return (
    <AriaListBox
      ...
      className={composeTailwindRenderProps(
        props.className,
        "outline-0 p-1 w-[200px] bg-white dark:bg-neutral-900 border-4 border-neutral-300 dark:border-neutral-700 rounded-lg ",
      )}
    >
      {children}
    </AriaListBox>
...
export const dropdownItemStyles = tv({
  base: "group flex items-center gap-4 cursor-pointer select-none py-2 pl-3 pr-3 selected:pr-1 rounded-lg outline outline-0 text-sm forced-color-adjust-none no-underline [&[href]]:cursor-pointer [-webkit-tap-highlight-color:transparent]",
  variants: {
    isDisabled: {
    ...
    isFocused: {
      true: "bg-p-900 dark:bg-p-900 text-white forced-colors:bg-[Highlight] forced-colors:text-[HighlightText]",
    },
  },
  compoundVariants: [
    {
  ...
  return (
    ...
    >
      {composeRenderProps(props.children, (children, { isSelected }) => (
        <>
          <span className=" text-black flex items-center flex-1 gap-2 font-normal truncate group-selected:font-semibold">
            {children}
          </span>
