...
import type { WordDataQuizExtended, QuizTypes } from "../../types/types";
...
function Quiz({
  ...
  const isAnsweredFunction = (word: WordDataQuizExtended) => {
    if (quizType === "multipleChoice") {
      return word.answer !== "";
    }
    if (quizType === "writeDefinition") {
      return word.writenDefinitionSubmited;
    }
    return false;
  };
  const isCorrectAnswerFunction = (word: WordDataQuizExtended) => {
    if (isAnsweredFunction(word)) {
      if (quizType === "multipleChoice") {
        return word.answer === word.correctAnswer;
      }
      if (quizType === "writeDefinition") {
        return word.writenDefinitionIsCorrect;
      }
    }
    return false;
  };
        ...
        <div className=" bg-p-200 p-1.5 py-1 rounded-md">
          <div className="flex items-center gap-0 ">
                    ...
                    >
                      <div
                        ...
                        className={clsx(
                          ...
                          !isAnsweredFunction(words[index]) && "bg-p-100",
                          !isAnsweredFunction(words[index]) &&
                            lastAnsweredIndex > index &&
                            " border-gray-300",
                          isCorrectAnswerFunction(words[index]) &&
                            "bg-green-400",
                          isAnsweredFunction(words[index]) &&
                            !isCorrectAnswerFunction(words[index]) &&
                            "bg-red-400",
            ...
            <div className="tracking-widest w-12 text-white text-sm font-medium text-right">
              {words.filter((word) => isAnsweredFunction(word)).length}/
              {amountOfQuestions}
            </div>
          </div>
        </div>
