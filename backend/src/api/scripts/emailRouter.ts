...
//   const user = await UserModel.findOne({ email: "nils.lorentzon@hotmail.com" });
//   if (!user) {
...
//     .replace(
//       "replacereplace",
//       `${API_URL}/authentication/verify-email?token=${user.verificationToken}`,
//     );
...
// emailRouter.post("/double-optin/subscribe", async (req: Request, res: Response) => {
...
// emailRouter.post("/double-optin/webhook", async (req: Request, res: Response) => {
//   const webhookSecret = process.env.RESEND_WEBHOOK_SECRET;
//   if (!webhookSecret) {
//     res.status(500).json({ error: "Webhook secret not configured" });
//     return;
